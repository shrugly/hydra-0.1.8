...
void create_common_env() {
  ...
  common_cgi_env[ix++] = env_gen_extra("GATEWAY_INTERFACE", CGI_VERSION, 0);
...
/* NCSA added */
  ...
  for (i = 0; i < ix; ++i) {
    if (common_cgi_env[i] == NULL) {
      log_error_time();
      fprintf(stderr, "Unable to allocate a component of common_cgi_env - out "
                      "of memory.\n");
      exit(1);
    }
  }
...
int init_cgi(request *req) {
  ...
  if (req->is_cgi == NPH || req->is_cgi == CGI || req->is_cgi == CGI_ACTION) {
  ...
  case 0:
    ...
    if (req->is_cgi == CGI || req->is_cgi == NPH || req->is_cgi == CGI_ACTION) {
      ...
      *c = '\0';
      ...
      newpath[0] = '.';
      newpath[1] = '/';
      memcpy(&newpath[2], req->pathname,
             l - 2); /* includes the trailing '\0' */
