...
#ifdef ENABLE_SMP
static pthread_mutex_t accept_mutex[2] = {PTHREAD_MUTEX_INITIALIZER,
...
void get_request(server_params *params, socket_type *server_s) {
  ...
  if ((!server_s->secure && total_global_connections[0] >= max_connections) ||
      (server_s->secure &&
       total_global_connections[1] >= max_ssl_connections)) {
  ...
  if (system_bufsize == 0) {
    ...
    if (getsockopt(conn->fd, SOL_SOCKET, SO_SNDBUF, &system_bufsize, &len) ==
            0 &&
        len == sizeof(system_bufsize)) {
      ;
    } else {
  ...
  if (system_bufsize < params->sockbufsize) {
    if (setsockopt(conn->fd, SOL_SOCKET, SO_SNDBUF,
                   (void *)&params->sockbufsize,
...
static void free_request(server_params *params, request **list_head_addr,
  ...
  for (i = COMMON_CGI_COUNT; i < req->cgi_env_index; ++i) {
    if (req->cgi_env[i]) {
      ...
      log_error_time();
      fprintf(stderr,
              "Warning: CGI Environment contains NULL value"
              "(index %d of %d).\n",
              i, req->cgi_env_index);
    }
  }
  ...
  if ((req->keepalive == KA_ACTIVE) && (req->response_status < 500) &&
      req->kacount > 0) {
    int bytes_to_move;
    ...
    if (bytes_to_move) {
      memcpy(conn->client_stream, req->client_stream + req->parse_pos,
             bytes_to_move);
 ...
 */
inline static int init_http_version_specific_stuff(request *req) {
 ...
 */
static int parse_range(const char *value, off_t *val1, off_t *val2) {
  int len;
  char *p;
  ...
  *value++ = '\0'; /* overwrite the : */
  ...
  case 'C':
    if (!memcmp(line, "CONTENT_TYPE", 13) && !req->content_type)
      ...
      req->keepalive =
          (!strncasecmp(value, "Keep-Alive", 10) ? KA_ACTIVE : KA_STOPPED);
    } else
      goto just_add_header;
