...
extern int ssl_verify; /* 0 no verify, 1 request certificate, and validate
                        ...
                        * 3 is request one, and try to verify it. Does not fail
                        * in any case.
                        */
...
static int generate_dh_primes(gnutls_dh_params *dh_params) {
  ...
  log_error_time();
  fprintf(stderr, "tls: Generated Diffie Hellman parameters [%d bits].\n",
          ssl_dh_bits);
...
static int generate_rsa_params(gnutls_rsa_params *rsa_params) {
  ...
  log_error_time();
  fprintf(stderr, "tls: Generated temporary RSA parameters.\n");
...
static int parse_cs_string(const char *string, const char *algo) {
  ...
  if (strlen(string) > sizeof(list) - 1)
    return 0;
...
int initialize_ssl(void) {
  ...
  if (ca_cert != NULL &&
      gnutls_certificate_set_x509_trust_file(credentials[0], ca_cert,
...
void ssl_regenerate_params(void) {
  ...
  if (!credentials[_cur]) {
    ...
    if (gnutls_certificate_set_x509_key_file(credentials[_cur], server_cert,
                                             server_key,
                                             GNUTLS_X509_FMT_PEM) < 0) {
      log_error_time();
    ...
    if (ca_cert != NULL &&
        gnutls_certificate_set_x509_trust_file(credentials[_cur], ca_cert,
                                               GNUTLS_X509_FMT_PEM) < 0) {
      log_error_time();
      fprintf(stderr, "tls: could not find '%s'.\n", ca_cert);
      exit(1);
  ...
  if (need_rsa_params) {
    ...
    gnutls_certificate_set_rsa_export_params(credentials[_cur],
                                             _rsa_params[_cur]);
  }
...
int send_alert(request *current) {
  ...
  retval = gnutls_alert_send(current->ssl_state, GNUTLS_AL_FATAL,
                             current->alert_to_send);
...
int initialize_ssl(void) {
  log_error_time();
  fprintf(stderr, "tls: SSL is not available in this build. Disable SSL in "
                  "Hydra's configuration file.\n");
  exit(1);
}

void ssl_reinit() { return; }
