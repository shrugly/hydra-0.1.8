...
#define GNUTLS_MAX_SESSION_ID 32
  ...
  if (!add_cgi_env(
          req, "SSL_PROTOCOL",
          gnutls_protocol_get_name(gnutls_protocol_get_version(req->ssl_state)),
          0))
    return 0;
...

  {
    ...
    ret =
        gnutls_server_name_get(req->ssl_state, str_session_id, &size, &type, 0);
  ...
  if (!add_cgi_env(req, "SSL_VERSION_INTERFACE", SERVER_NAME "/" SERVER_VERSION,
                   0))
    return 0;

  if (!add_cgi_env(
          req, "SSL_CIPHER",
          gnutls_cipher_suite_get_name(gnutls_kx_get(req->ssl_state),
                                       gnutls_cipher_get(req->ssl_state),
                                       gnutls_mac_get(req->ssl_state)),
          0))
    return 0;

  {
    ...
    char *p;
    int keysize =
        gnutls_cipher_get_key_size(gnutls_cipher_get(req->ssl_state)) * 8;
  ...
  if (gnutls_session_get_id(req->ssl_state, session_id, &session_id_length) >=
      0) {
...

  {
    ...
    if (ssl_verify >= 1) {
       ...
       */
      cert_list = gnutls_certificate_get_peers(req->ssl_state, &cert_list_size);
