...
int modified_since(time_t mtime, char *if_modified_since) {
   ...
   */
  else if (sscanf(if_modified_since, "%d %10s %d %d:%d:%d GMT", &day, monthname,
...
void create_url(char *buffer, int buffer_size, int secure, const char *hostname,
                int port, const char *request_uri) {
  char *proto;
  char str_port[23];
  int do_port = 0;
 ...
 */
void break_comma_list(char *etag, char *broken_etag[MAX_COMMA_SEP_ELEMENTS],
                      int *elements) {
  char *p = etag;
