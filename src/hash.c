...
static unsigned four_char_hash(const char *buf) {
  unsigned int hash =
      (buf[0] +
       (buf[1] ? buf[1]
               : 241 + (buf[2] ? buf[2] : 251 + (buf[3] ? buf[3] : 257))));
  DEBUG(DEBUG_HASH) {
    log_error_time();
...
static unsigned sdbm_hash(const char *str) {
  ...
  DEBUG(DEBUG_HASH) { fprintf(stderr, "%u\n", hash); }
  return hash;
}
#else
#ifdef WANT_DJB2_HASH
...
static hash_struct *hash_insert(hash_struct *table[], unsigned int hash,
                                char *key, char *value) {
  ...
  DEBUG(DEBUG_HASH)
  fprintf(stderr, "Adding key \"%s\" for value \"%s\" (hash=%d)\n", key, value,
          hash);
...
static hash_struct *find_in_hash(hash_struct *table[], char *key,
                                 unsigned int hash) {
  hash_struct *current;
...
static void clear_hashtable(hash_struct *table[], int size) {
  int i;
  hash_struct *temp;
...
void dump_mime(void) { clear_hashtable(mime_hashtable, MIME_HASHTABLE_SIZE); }
