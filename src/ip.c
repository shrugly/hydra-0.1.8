...
int bind_server(int server_s, char *server_ip, int port) {
#ifdef INET6
...
#ifdef INET6
  if (getnameinfo((struct sockaddr *)s, sizeof(struct SOCKADDR), dest, len,
...
#else
#ifdef ENABLE_SMP
  /* This could also be used for IPv6 addresses, but I'm not
   * touching that.
   */
  inet_ntop(AF_INET, &s->sin_addr, dest, len);
#else
  memcpy(dest, inet_ntoa(s->sin_addr), len);
#endif
#endif
  return dest;
}
...
int net_port(struct SOCKADDR *s) {
  ...
  if (getnameinfo((struct sockaddr *)s, sizeof(struct SOCKADDR), NULL, 0, serv,
