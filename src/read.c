...
int read_header(server_params *params, request *req) {
  ...
  if (req->status < BODY_READ) {
    ...
    bytes = socket_recv(req, buffer + req->client_stream_pos, buf_bytes_left);
...
int write_body(request *req) {
  ...
  bytes_written =
      write(req->post_data_fd.fds[1], req->header_line, bytes_to_write);
  ...
  if (bytes_written == -1) {
    if (errno == EWOULDBLOCK || errno == EAGAIN)
      ...
      boa_perror(req,
                 "write body"); /* OK to disable if your logs get too big */
      return 0;
    } else {
      boa_perror(req,
                 "write body"); /* OK to disable if your logs get too big */
