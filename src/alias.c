...
alias *find_alias(char *hostname, char *uri, int urilen) {
  ...
  while (current) {
    ...
    fprintf(stderr,
            "%s:%d - comparing \"%s\" (request) to \"%s\" (alias): ", __FILE__,
            __LINE__, uri, current->fakename);
#endif
    /*
    ...
    if (current->fake_len <= urilen &&
        ...
        (current->fakename[current->fake_len - 1] == '/' ||
         (current->fakename[current->fake_len - 1] != '/' &&
          (uri[current->fake_len] == '\0' || uri[current->fake_len] == '/')))) {
#ifdef FASCIST_LOGGING
...
int is_executable_cgi(request *req, const char *filename) {
  ...
  if (strcmp(CGI_MIME_TYPE, mime_type) == 0) { /* cgi */
    int ret;
    ret =
        init_local_cgi_stuff(req, (req->http_version == HTTP_0_9) ? NPH : CGI);

    if (ret == 0)
      return -1;

    return 1;
  } else if ((hic = find_cgi_action_appr_module(mime_type, 0)) !=
             0) { /* CGI ACTION */
    int ret = 0;
...
int translate_uri(request *req) {
  ...
  char *p;
  int uri_len, ret, len;
      /* FIXME-andreou */ /* Goes in pair with the one at L263 */
  ...
  if (current) {
    ...
    memcpy(buffer + current->real_len, req->request_uri + current->fake_len,
...
int init_script_alias(request *req, alias *current1, int uri_len) {
  ...
  if (uri_len - current1->fake_len + current1->real_len > MAX_HEADER_LENGTH) {
    log_error_doc(req);
    fputs("uri too long!\n", stderr);
    send_r_bad_request(req);
    return 0;
  ...
  memcpy(pathname + current1->real_len, &req->request_uri[current1->fake_len],
         uri_len - current1->fake_len + 1); /* the +1 copies the NUL */
#ifdef FASCIST_LOGGING
  log_error_time();
  fprintf(stderr, "%s:%d - pathname in init_script_alias is: \"%s\" (\"%s\")\n",
  ...
  /* we have path_info if c == '/'... still have to check for query */
     ...
     */
    req->script_name[strlen(req->script_name) - path_len] =
        '\0'; /* zap off the /foo part */
    ...
    while (current && !req->path_translated) {
      if (!strncmp(req->path_info, current->fakename, current->fake_len)) {

        if (current->real_len + path_len - current->fake_len >
            MAX_HEADER_LENGTH) {
          log_error_doc(req);
          fputs("uri too long!\n", stderr);
          send_r_bad_request(req);
        ...
        memcpy(buffer, current->realname, current->real_len);
        strcpy(buffer + current->real_len, &req->path_info[current->fake_len]);
