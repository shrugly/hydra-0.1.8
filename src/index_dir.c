...
#define INT_TO_HEX(x) ((((x)-10) >= 0) ? ('A' + ((x)-10)) : ('0' + (x)))
...
int index_directory(char *dir, char *title) {
  ...
  for (i = 0; i < numdir; ++i) {
    ...
    if (html_escape_string(dirbuf->d_name, html_filename, NAMLEN(dirbuf)) ==
        NULL) {
      send_error(4);
      return -1;
    }
    if (http_escape_string(dirbuf->d_name, http_filename, NAMLEN(dirbuf)) ==
        NULL) {
      send_error(4);
      return -1;
    }
    ...
    printf("<tr>"
           ...
           "</tr>\n",
           http_filename, html_filename, ctime(&statbuf.st_mtime),
...
#ifdef USE_LOCALTIME
  printf("</table>\n<hr noshade>\nIndex generated %s %s\n"
         "<!-- This program is part of the Boa Webserver Copyright (C) "
         "1991-2002 http://www.boa.org -->\n"
         "</body>\n</html>\n",
