 ...
 *  Some changes Copyright (C) 1997 Alain Magloire
 * <alain.magloire@rcsm.ee.mcgill.ca> Portions Copyright (C) 2002 Nikos
...
#define SET_PTH_SIGFLAG(flag, val) global_server_params[0].flag = val
...
void sigterm(int dummy) { SET_PTH_SIGFLAG(sigterm_flag, 1); }
...
void sigterm_stage2_run() { /* lame duck mode */
  ...
  log_error_time();
  fprintf(stderr, "exiting Hydra normally (uptime %d seconds)\n",
...
void sighup(int dummy) { SET_LOCAL_PTH_SIGFLAG(sighup_flag, 1); }
    ...
    for (i = 1; i < global_server_params_size; i++) {
      if ((ret = pthread_kill(global_server_params[i].tid, SIGHUP)) != 0) {
        ...
        fprintf(stderr, "Could not kill thread: %d. errno = %d.\n",
                (int)global_server_params[i].tid, ret);
      } else if ((ret = pthread_join(global_server_params[i].tid, NULL)) != 0) {
        log_error_time();
...
void sigchld(int dummy) { SET_LOCAL_PTH_SIGFLAG(sigchld_flag, 1); }
...
void sigalrm(int dummy) { SET_PTH_SIGFLAG(sigalrm_flag, 1); }

void sigusr2(int dummy) { return; }

void sigusr1(int dummy) { SET_PTH_SIGFLAG(sigusr1_flag, 1); }
...
void sigusr1_run(void) {
  ...
  for (i = 0; i < global_server_params_size; i++) {
    ...
    fprintf(stderr, "Thread %d: %ld requests, %ld errors\n", i + 1,
            global_server_params[i].status.requests,
