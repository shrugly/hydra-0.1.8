...
void reset_output_buffer(request *req) { req->buffer_end = 0; }
...
int req_flush(request *req) {
  ...
  if (bytes_to_write) {
    ...
    bytes_written =
        socket_send(req, req->buffer + req->buffer_start, bytes_to_write);
    ...
    fwrite(req->buffer + req->buffer_start, sizeof(char), bytes_written,
           stderr);
